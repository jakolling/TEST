...
def create_pizza_chart(params=None, values_p1=None, values_p2=None, values_avg=None, 
    ...
    try:
        ...
        try:
            ...
            print(f"Tentando adicionar logo ao gráfico pizza simples")
            
            try:
                # Usar uma abordagem completamente diferente: adicionar imagem diretamente usando AnnotationBbox
                # Isso deve resolver problemas de sobreposição 
                
                # Primeiro, verificar se temos o logo como arquivo em attached_assets
                ...
                if os.path.exists(logo_file):
                    # Carregar imagem e criar um objeto OffsetImage
                    logo = mpimg.imread(logo_file)
                    imagebox = OffsetImage(logo, zoom=0.40)  # Zoom maior para o logo ser mais visível
                    
                    # Posicionar no centro do eixo polar
                    ab = AnnotationBbox(imagebox, (0, 0),  # O centro (0,0) do gráfico polar
                                       xycoords='data',    # Coordenadas de dados em vez de figura
                                       frameon=False,      # Sem borda
                                       zorder=1000)        # Colocar na frente de tudo
                    
                    # Adicionar a anotação diretamente ao eixo para ficar no centro do gráfico polar
                    ax.add_artist(ab)
                    
                    print("Logo adicionado com sucesso usando AnnotationBbox")
                else:
                    print(f"Arquivo do logo não encontrado em: {logo_file}")
            except Exception as e:
                print(f"Erro ao adicionar logo usando AnnotationBbox: {str(e)}")
                import traceback
    ...
    try:
        ...
        try:
            ...
            if os.path.exists(logo_path):
                ...
                print("Logo do Vålerenga carregado com sucesso no gráfico simples")
            else:
