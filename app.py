...
if uploaded_files:
    ...
    try:
        ...
        if not df.empty:
            ...
            min_min, max_min = 0, 1000

        minutes_range = st.sidebar.slider(
            'Minutes Played', 
            min_min, 
            max_min, 
            value=tuple(st.session_state.filters['minutes_range'])
        )
        st.session_state.filters['minutes_range'] = tuple(minutes_range)
        ...
        if not df_minutes.empty:
            min_mpg = int(df_minutes['Minutes per game'].min())
            max_mpg = int(df_minutes['Minutes per game'].max())
        else:
        ...
        mpg_range = st.sidebar.slider(
            ...
            min_mpg, 
            max_mpg, 
            value=tuple(st.session_state.filters['mpg_range'])
        )
        st.session_state.filters['mpg_range'] = tuple(mpg_range)
        df_minutes = df_minutes[df_minutes['Minutes per game'].between(*mpg_range)]

        # Filtro de Idade (Correção Aplicada)
        if not df_minutes.empty:
            min_age = int(df_minutes['Age'].min())
            max_age = int(df_minutes['Age'].max())
        else:
            min_age, max_age = 16, 40

        age_range = st.sidebar.slider(
            'Age Range', 
            min_age, 
            max_age, 
            value=tuple(st.session_state.filters['age_range'])
        )
        st.session_state.filters['age_range'] = tuple(age_range)
        df_minutes = df_minutes[df_minutes['Age'].between(*age_range)]

        # Filtro de Posições
        if 'Position' in df_minutes.columns and not df_minutes.empty:
            ...
            sel_pos = st.sidebar.multiselect(
                'Positions', 
                all_pos, 
                default=st.session_state.filters['sel_pos']
            )
        ...
        players = sorted(df_minutes['Player'].unique()) if not df_minutes.empty else []
        ...
        with tabs[0]:
            ...
            if 6 <= len(sel) <= 12 and not df_minutes.empty:
                try:
                    ...
                    st.error("Selected players not found in filtered data")
        ...
        with tabs[1]:
            ...
            if len(selected_metrics) >= 1 and not df_minutes.empty:
                fig = make_subplots(
                ...
                for idx, metric in enumerate(selected_metrics, 1):
                    try:
                        ...
                        st.error(f"Data not available for {metric}")
                        continue
        ...
        with tabs[4]:
            ...
            if len(sel) >= 2 and not df_minutes.empty:
        ...
        with tabs[5]:
            ...
            if not df_minutes.empty:
                ...
                st.warning("No data available for PCA analysis")
