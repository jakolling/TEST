# Football Analytics App - Enhanced PCA Version
# Full code without interruptions - v4.1
...
st.caption("Created by João Alberto Kolling | Player Analysis System v4.1")
...
with st.expander("📘 User Guide & Instructions", expanded=False):
    ...
    **⚠️ Requirements:**  
    ...
    `pip install kaleido==0.2.1.post1 xlsxwriter statsmodels`  
    2. Data must contain columns: Player, Age, Position, Metrics, Team  
    
    **Key Features:**  
    - Enhanced PCA analysis with biplots  
...
if uploaded_files:
    ...
    try:
        ...
        tabs = st.tabs(['Radar', 'Bars', 'Scatter', 'Profiler', 'Correlation', 'Advanced PCA'])
        ...
        # Enhanced PCA Analysis (Tab 6)
        with tabs[5]:
            st.header('Advanced PCA Analysis + Excel Export')
            performance_cols = [col for col in numeric_cols if col not in ['Age','Height','Country','Minutes played','Position']]
            
            # PCA Controls
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                pca_type = st.selectbox('PCA Type', ['linear', 'kernel'], 
                                       help="Linear PCA for interpretability, Kernel PCA for non-linear relationships")
            with col2:
                n_components = st.number_input('Number of Components', 1, 5, 2, 
                                              disabled=(pca_type=='kernel'))
            with col3:
                kernel_type = st.selectbox('Kernel Type',['linear','rbf'], index=1, 
                                          disabled=(pca_type=='linear'))
            with col4:
                gamma = st.number_input('Gamma', value=0.1, min_value=0.0, step=0.1, 
                                        disabled=(pca_type=='linear' or kernel_type=='linear'))

            # VIF Filtering
            with st.expander("🔍 Advanced Feature Selection"):
                vif_threshold = st.slider('VIF Threshold', 1, 10, 5,
                                         help="Remove features with multicollinearity (VIF > threshold)")
            ...
            # VIF Calculation
            if sel:
                X = df_minutes[sel].dropna()
                vif = pd.Series(
                    [variance_inflation_factor(X.values, i) for i in range(X.shape[1])],
                    index=X.columns
                )
                sel = [col for col in sel if vif[col] <= vif_threshold]
                st.info(f"Selected features after VIF filtering: {', '.join(sel)}")

            if len(sel) < 2:
                st.error('Insufficient metrics after VIF filtering. Relax VIF threshold or select different metrics.')
                st.stop()
            ...
            class EnhancedPCA:
                ...
                def fit_transform(self, X, weights):
                    ...
                    scores = self.pca.fit_transform(Xw)
            ...
            try:
                pca = EnhancedPCA(n_components=n_components, 
                                 kernel=kernel_type if pca_type=='kernel' else 'linear',
                                 gamma=gamma)
                
                df_sel = df_minutes[sel].dropna()
                weights = pd.Series([1.0]*len(sel), index=sel)  # Equal weights
                
                scores = pca.fit_transform(df_sel, weights)
                idx = df_sel.index
                
                df_pca = pd.DataFrame({
                    'Player': df_minutes.loc[idx, 'Player'],
                    'Team': df_minutes.loc[idx, 'Team'],
                    'PCA Score': scores[:,0] if n_components >=1 else np.zeros(len(idx)),
                ...
                # Variance Analysis
                if pca_type == 'linear' and n_components > 1:
                ...
                if pca_type == 'linear' and n_components >= 2:
                    st.subheader('Biplot Visualization')
                    ...
                    for i, feature in enumerate(sel):
        ...
        with st.expander("ℹ️ How to interpret PCA results"):
            st.markdown("""
            **Interpretation Guide:**
            1. **PCA Scores:** Standardized values representing relative performance
            2. **Explained Variance:** % of total information captured by each component
            3. **Biplot:** 
               - Points: Players in PCA space
               - Arrows: Direction and strength of metrics
            4. **VIF:** Values >5 indicate multicollinearity
            """)
    ...
    st.warning("⚠️ Required: `pip install kaleido==0.2.1.post1 statsmodels`")# Football Analytics App - Complete Version
